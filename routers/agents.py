...
async def direct_chat(
    ...
    if cached_answer:
        ...
        return {
            ...
            "suggestions" : _get_gtcc_suggestions(topic) if request.suggest else [],
            "cached"      : True,
        }
    ...
    try:
        ...
        event = await _save_chat_event(db, current_user, request, answer, elapsed_ms, clean_query)

        from routers.rag import _get_gtcc_suggestions
        suggestions = _get_gtcc_suggestions(topic) if request.suggest else []

        logger.info(f"[Direct Chat] Session: {request.session_id} | lang={lang} | Q: '{clean_query[:60]}' | Topic: {topic} | Time: {elapsed_ms}ms")
        ...
        try:
            event = await _save_chat_event(db, current_user, request, fallback, 0)
            from routers.rag import _get_gtcc_suggestions
            return {
                "result"      : fallback,
                "event_id"    : event.id,
